...
模型和非敏感API配置保存在独立运行settings.json。API Key用`configure-api --kind image|text --model "模型名"`在本地隐藏提示中输入；Windows写入当前用户环境变量并接入当前进程，其他宿主使用其安全凭据存储。命令不接受`--key`参数，不把Key放进命令行、聊天、普通JSON或输出。随后Agent查官方文档，用configure补齐技术配置并运行本地检查，直到可用或明确说明协议不受支持。
