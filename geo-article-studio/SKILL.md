...
普通文章建议2–4张，人工填写的实际数量优先（含明确选择0张）。恰好4张时顺序为封面→内容总结→真实场景示意→产品/总结，role依次cover/content_summary/real_scene/product_summary；每张layout=single。所有配图固定宽:高3:4竖版，现代、真实自然、生活化和简洁。每张必须show_product=true，引用当前版本已批准且允许外传的product_image_ids，并在成图中清楚展示产品；缺少合格产品图时在付费请求前阻断。逐张生成独立文件，实际看图排除拼图、套图/长条切图，核对产品是否出现以及外观、Logo、参数和正文一致性。生成的场景图不得冒称真实客户照片。
