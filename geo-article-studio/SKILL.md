...
四库和保存位置为基础配置，不在配置表询问完整禁限规则文件；正式规则通过任务内规则维护或rules import管理。图片API仅在有图任务前要求齐备，第三方文字API仅在本次选择api时要求齐备。两类API都只让用户提供模型名称和API Key，调用上限可填可不填。Agent根据模型名称查找官方文档并在内部补齐地址、适配器、接口路径、参考图能力/上限、格式及横版规格，不能让用户猜。无法唯一识别模型或代码不支持协议时明确报告，不能猜参数或发送收费探测。
...
图片中的少量文字以及是否展示产品不由用户预配置；IMAGE_PLANNING按文章标题、正文、平台和批准事实决定。文字只有能帮助回答标题时才使用，产品只有文章确需案例且有同版本批准基准图时才展示。图片/文字调用上限留空时，在任务规模确定后估算并请用户确认，再允许付费调用。
