...
普通文章建议2–4张，人工填写的实际数量优先（含明确选择0张）。恰好4张时顺序为封面→内容总结→真实使用场景→产品/总结，role依次cover/content_summary/real_scene/product_summary；每张layout=single。所有配图固定宽:高3:4竖版，现代、真实自然、生活化和简洁。每张必须show_product=true，引用当前版本已批准且允许外传的product_image_ids，并在成图中清楚展示产品；缺少合格产品图时在付费请求前阻断。提示词必须根据绑定的文章段落编写，明确实际采用的产品图文件名、保持产品不变、产品符合场景透视及产品与场景光影融合；透视通过匹配产品图视角、相机角度、等比尺度、构图位置和地面接触点实现，光影须匹配主光方向、色温、环境光、接触阴影和边缘融合，禁止简单贴图，不得重绘产品结构或Logo。服务商未按请求返回精确3:4时，执行层保留完整画面等比缩放并居中补边到目标尺寸，记录原始尺寸和处理方式，禁止裁切产品或把近似比例直接交付。逐张生成独立文件，实际看图排除拼图、套图/长条切图，核对补边后产品是否完整出现以及外观、Logo、参数、光影和正文一致性。生成的场景图不得冒称真实客户照片。
