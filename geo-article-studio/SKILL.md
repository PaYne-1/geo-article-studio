...
四库和保存位置为基础配置，不在配置表询问完整禁限规则文件；用户确认的GEO标准随技能内置，自定义规则可通过任务内规则维护或rules import追加。图片API仅在有图任务前要求齐备，第三方文字API仅在本次选择api时要求齐备。两类API都只让用户提供模型名称和API Key，调用上限可填可不填。Agent根据模型名称查找官方文档并在内部补齐地址、适配器、接口路径、参考图能力/上限、格式及横版规格，不能让用户猜。无法唯一识别模型或代码不支持协议时明确报告，不能猜参数或发送收费探测。
