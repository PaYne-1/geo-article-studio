...
这些标准必须在图片生成请求发出前落实。规划时逐张核对角色、正文段落、事实依据、构图、横版尺寸、允许文字、产品展示条件和参考图授权，再把全部限制写入该张prompt；不把关键限制留给生成后的视觉审核。提交计划前逐张确认prompt不会要求模型虚构Logo、型号、产品外观、参数、客户实拍或正文没有的功能。无法在生成前形成合规prompt时应阻断该图，不能先生成再碰运气。
