...
当前 Hermes 会话的 Qwen 完成语义分析、规划、正文、语义审核和复盘；Python完成来源验证、有限检索、状态、结构校验、请求上限、图片 API 和文件落盘。上述为选择宿主模型的路径；v1.3可在开始任务后选择已配置的第三方文字API，详见docs/文字模型配置与选择.md。宿主模式无需额外文字 API 密钥，不导入未公开 Hermes 模型私有函数，不让用户逐步手写模型结果 JSON。开始任务中，用户完整列出目标、平台或目标AI的所有展示候选项，就是该字段的全选；宿主直接把完整数组写入brief并继续，不能为此调用Hermes原生提问/选项控件。只在用户只列部分候选、漏字段或写入未知项时补问。
...
图片动作使用选定参考图与已批准产品图。默认reviewed模式视觉审核必须实际读取本轮生成文件，记录所用真实图片和能力；用户本轮明确选择direct_use时跳过视觉审核且成品注明未做视觉审核，不得伪造reviewer、viewed_image_ids或视觉通过记录；仅模型配置写了 supports_vision、拿到图片 URL、检查尺寸或生成端口通畅都不能代替视觉验收。
