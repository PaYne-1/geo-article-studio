...
def form_for(trigger,settings,provided=None,*,config_path=None,persisted=None):
    ...
    if action=='configure_task':
        ...
        return result
    values={'product':product,'mode':defaults.get('mode'),'text_source':None,'image_review_policy':'reviewed','chat_scope':'当前产品相关记录及标注的通用品类记录','extra_requirements':''}; required=['product','mode','text_source']
